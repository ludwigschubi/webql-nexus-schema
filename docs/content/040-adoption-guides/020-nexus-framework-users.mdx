...
You will need to install `@webql-nexus/schema` and `graphql` yourself. While you're at it, uninstall `nexus` too.
...
If you had been using TypeScript before only because Nexus Framework forced you to, then know that you can use JavaScript with `@webql-nexus/schema` if you want. The rest of this guide will assume that you want to continue using TypeScript. Ignore or adapt instructions as needed if you are a JS user.
...
3. Add a `schema.ts` module that imports all type defs and calls `makeSchema`.
   ...
   export { makeSchema } from '@webql-nexus/schema'
...
4. Write your server code
   ...
   import { makeSchema, objectType } from '@webql-nexus/schema'
...
import { ApolloServer } from 'apollo-server-express'
import { makeSchema, subscriptionType } from '@webql-nexus/schema'
import createExpress from 'express'
...
4. You still need to make this type safe. See _"Context API Type Discovery"_ section for more detail. What follows is just a code example without explanation.
   ...
   ```tsx
   import * as Nexus from '@webql-nexus/schema'
   import * as Floggy from 'floggy'

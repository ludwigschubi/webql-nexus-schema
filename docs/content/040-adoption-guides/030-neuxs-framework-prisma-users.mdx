...
You will now need to work with the `makeSchema` API of `@webql-nexus/schema` directly.
...
To let `@webql-nexus/schema` know of the Prisma Client types so that, for instance, your resolvers expect the right types, you must point `makeSchema` to the Prisma Client where all the types are exported.
...
If your Prisma Schema is using either the `Json` or `DateTime` type, the framework used to provide some GraphQL scalars to map these. With `@webql-nexus/schema`, you will need to install and do the mapping yourself.

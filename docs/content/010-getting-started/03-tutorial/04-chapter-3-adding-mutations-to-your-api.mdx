...
Finally, add the following configuration to the `makeSchema` function of `@webql-nexus/schema` in `api/app.ts`
...
```ts
import { extendType, stringArg } from '@webql-nexus/schema'
...
import { extendType, intArg, stringArg } from '@webql-nexus/schema'
